...
% Solution polishing at the $i^{th}$ location first defines a random path through $\tilde{\mathbf{y}}_{i}$. For a set number of iterations, $j$, each initial imputed value is perturbed by a small amount $p_{m}^{(j)}$:
% \begin{align}
%     p_{m}^{(j)}                  & = -t_{2} + r_{\in[0,1]} \times (t_{2} - (-t_{2}))                         \\
%     \tilde{y}_{m,i}^{(j)}        & = \tilde{y}_{m,i} + p_{m}                                                 \\
%     \tilde{\mathbf{y}}_{i}^{(j)} & = \left\{ \dots, \tilde{y}_{m,i}^{(j)}, \dots \right\}                    \\
%     d_{2}                        & = \left|\mathcal{F}_{\theta}(\tilde{\mathbf{y}}_{i}^{(j)}) - z_{i}\right|
% \end{align}

% Where $r_{\in[0,1]}$ is a random uniform number between 0 and 1 and $t_{2}$ is the second rejection tolerance; this amounts to generating a uniform random perturbation $\in [-t_{2}, t_{2}]$. $\tilde{y}_{m,i}^{(j)}$ is the trial latent value and $\tilde{\mathbf{y}}_{i}^{(j)}$ is the trial latent vector that contains the perturbed value and any other previously accepted perturbations. $d_{2}$ is the absolute difference between the evaluated trial latent vector and the observed value. After a factor is perturbed $\tilde{z_{i}}^{(j)} = \mathcal{F}_{\theta}\left(\tilde{\mathbf{y}}_{i}^{(j)}\right)$ and is reevaluated. If  $\left|\tilde{z_{i}}^{(j)}-z_{i}\right|$ is less than $d_{2}$, the perturbed value is kept; otherwise, it is rejected, and the value reverts to the previous iteration:
% \begin{equation}
%     \tilde{y}_{m,i} =
%     \begin{cases}
%         \tilde{y}_{m,i}^{(j)},   & \text{ if } \left|\tilde{z_{i}}^{(j)}-z_{i}\right| < d_{2} \\
%         \tilde{y}_{m,i}^{(j-1)}, & \text{ otherwise }                                         \\
%     \end{cases}
% \end{equation}

% If at any point $d_{2} < t_{2}$ the loop breaks as $\mathcal{F}_{\theta}(\tilde{\mathbf{y}}_{i}^{(j)}) \approx z_{i}$. Practice shows that $\approx 0.01$ is a reasonable tolerance for $t_2$.
