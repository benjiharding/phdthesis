...
The algorithm begins by determining a random or semi-random path through all data locations to be imputed. At each imputation location, the conditional moments for each latent covariance structure are solved using the normal equations:
\begin{align}
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \State $d_{1} = |\tilde{z}_{i}^{(j)} - z_{i}|$ \Comment{Compare to observed value}
        \If{$j > iter1$}
        \State break
        \EndIf
...
\begin{figure}
    ...
    \caption{Plan (a), east-west (b) and north-south (c) sections through the first \gls{NMR} realization mapped to Gaussian space.}
...
\begin{figure}[htb!]
    \centering
    \includegraphics[scale=0.7, max size={\textwidth}{\textheight}]{./0-Figures/05-Ch5/repro_gridded_data.png}
    \caption{}
    \label{fig:repro_gridded_data}
\end{figure}
...
\begin{figure}
    ...
    \caption{Plan (a), east-west (b) and north-south (c) sections through the first \gls{NMR} realization mapped to original units.}
    \label{fig:nmr_au_real}
\end{figure}

\begin{figure}[htb!]
    \centering
    \includegraphics[scale=0.7, max size={\textwidth}{\textheight}]{./0-Figures/05-Ch5/repro_nmr_orig_cdf.png}
    \caption{}
    \label{fig:repro_nmr_orig_cdf}
\end{figure}

\begin{figure}[htb!]
    \centering
    \includegraphics[scale=0.7, max size={\textwidth}{\textheight}]{./0-Figures/05-Ch5/repro_gridded_vario.png}
    \caption{}
