...
\begin{figure}[htb!]
    \centering
    \includegraphics[scale=0.5, max size={\textwidth}{\textheight}]{./0-Figures/03-Ch03/lightning_indicators.png}
...
Table \ref{tab:lightning_resources} summarizes the resources of the \gls{SMU} scale \gls{NMR} and \gls{SGS} models above the 0.9 quantile as a fraction of true resources. The resources assume the synthetic variable is measured in grams per tonne and priced in ounces. The \gls{NMR} model shows improvement in both tonnes and grade, leading to a 9\% increase in contained metal ounces relative to the \gls{SGS} model. The increase in tonnes above the true 0.9 quantile is attributed to the increase in east-west continuity imparted by factors two and three.

\begin{table}[!htb]
    \centering
    \caption{\Gls{SMU} scale resources above the 0.9 quantile as a fraction of the true resources. g/t=grams per tonne.}
