...
The remaining sections of this chapter discuss the calculation of high-order statistics and their use for measures of non-Gaussianity, details regrading the components of the \gls{NMR} framework, and finally, a small synthetic example of the complete \gls{NMR} workflow with highly structured extreme values.
...
The \gls{NMR} is a framework for the generation of non-Gaussian spatial fields. Characterizing non-Gaussian spatial features, like the connectivity of extreme values, requires statistics above the second order. It is shown that these higher order statistics such as the distribution of runs and the $n$-point connectivity function are able to differentiate non-Gaussian from Gaussian sequences. The \gls{NMR} is designed to overcome known shortcomings of the multivariate Gaussian \gls{RF} model, particularly with strongly positively skewed distributions. This is achieved through consideration of both two- and multi-point statistics in the generation of a probabilistic model. Examples are given highlighting spatial structures that cannot be reproduced with a two-point covariance structure and a multivariate Gaussian simulation algorithm. A core concept of the \gls{NMR} is that it is a spatial \gls{GMM}, where the mixing of Gaussian components results in a highly non-Gaussian output. Each component to be mixed is defined by unique covariance structure; this ``pool'' of components is designed such that certain components impart certain spatial features in the final model. The components input to the network are activated with a power law activation function. This non-linear activation function allows features of the latent factors to be emphasized in particular regions of the continuous grade range. That is, the low values and high values can have different spatial structure and different multi-point connectivity. This difference is continuity is something that cannot be easily achieved with two-point statistics alone. The spatial features of the final model are defined at the beginning of the modeling process. The practitioner specifies the goals of the model in terms of two-point and high order statistics. These goals include the normal score variogram model, indicator variogram models, cumulative run-length frequencies, and the $n$-point connectivity function. The parameters of the mapping function are inferred with stochastic optimization. These parameters result in a spatial mixture that reproduces the statistics outlined in the modeling goals. Generation of conditioning data for latent factor realizations is an imputation problem, and these imputed factors become conditioning data for gridded factor realizations. Finally, the gridded factors are mapped to observed space with the inferred parameters, resulting in the final gridded realizations with the correct high-order statistics.

A small example is shown highlighting the ability of the \gls{NMR} to capture connected extreme values and non-stationary features like changes in orientation. A well-designed Gaussian pool can accommodate multiple orientations and anisotropies. The model shows a significant improvement in contained metal relative to an \gls{SGS} model characterized by a two-point covariance structure. The following chapter presents the details of network components of the \gls{NMR} framework including latent factor design and parameter inference.