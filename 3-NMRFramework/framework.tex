...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[node distance=1.5cm, align=center]
        ...
        \tikzstyle{acceptBlock} = [rectangle, rounded corners, minimum width=4cm, minimum height=1.0cm,text centered, draw=black, fill=red!15]
        \tikzstyle{actionBlock} = [rectangle, rounded corners, text width=2.5cm, minimum height=1.0cm,text centered, draw=black, fill=black!5]

        % Specification of nodes (position, etc.)
        \node (start) [base] {Define Objectives};
        \node (poolBlock) [base, below of=start, yshift=-0.0cm] {Define Latent Pool};
        \node (inferBlock) [base, below of=poolBlock, yshift=-0.0cm] {Infer Parameters};
        \node (accept1Block) [acceptBlock, below of=inferBlock, yshift=-0.0cm] {Acceptable Objective Reproduction?};
        \node (latentBlock) [base, below of=accept1Block, yshift=-0.75cm] {Latent Imputation};
        \node (accept2Block) [acceptBlock, below of=latentBlock, yshift=-0.0cm] {Acceptable Latent Reproduction?};
        \node (simBlock) [actionBlock, below of=accept2Block, yshift=-0.75cm] {Gridded Latent Simulation};
        \node (mapBlock) [actionBlock, below of=simBlock, yshift=-0.0cm] {Map Latent to Observed Space};
        \node (accept3Block) [acceptBlock, below of=mapBlock, yshift=-0.0cm] {Acceptable Gridded Reproduction?};
        \node (end) [actionBlock, below of=accept3Block, yshift=-0.75cm] {Final Gridded Realizations};

        % Specification of lines between nodes specified above
        \draw [->] (start) -- (poolBlock);
        \draw [->] (poolBlock) -- (inferBlock);
        \draw [->] (inferBlock) -- (accept1Block);
        \draw [->] (accept1Block) -- node[anchor=east] {\textbf{yes}} (latentBlock);
        \draw [->] (accept1Block.west) -- ++(-0.75,0) node[midway, above] {\textbf{no}} coordinate(almostleft) |- (start.west);
        \draw [->] (almostleft|-poolBlock.170) -- (poolBlock.170);
        \draw [->] (almostleft|-inferBlock.170) -- (inferBlock.170);
        \draw [->] (latentBlock) -- (accept2Block);
        \draw [->] (accept2Block) -- node[anchor=east] {\textbf{yes}} (simBlock);
        \draw [->] (accept2Block.west) -- ++(-1.25,0) node[midway, above] {\textbf{no}} coordinate(almostleft) |-(poolBlock.190);
        \draw [->] (almostleft|-latentBlock.west) -- (latentBlock.west);
        \draw [->] (almostleft|-inferBlock.190) -- (inferBlock.190);
        \draw [->] (simBlock) -- (mapBlock);
        \draw [->] (mapBlock) -- (accept3Block);
        \draw [->] (accept3Block) -- node[anchor=east] {\textbf{yes}} (end);
        \draw [->] (accept3Block.east) -- ++(1.25,0) node[midway, above] {\textbf{no}} coordinate(almostright) |- (poolBlock.east);
        \draw [->] (almostright|-simBlock.east) -- (simBlock.east);
        \draw [->] (almostright|-latentBlock.east) -- (latentBlock.east);
        \draw [->] (almostright|-inferBlock.east) -- (inferBlock.east);
    \end{tikzpicture}
    \caption{High level flow chart illustrating the key components of the \gls{NMR} framework. There are multiple junctions where previous steps can be revisited and refined. Reverse-flow arrows leading to multiple actions emphasize the interconnected nature of each component.}
...
\begin{figure}[htb!]
    \centering
    \begin{tikzpicture}[node distance=1.5cm, align=center]
        \tikzstyle{mapBlock} = [rectangle, rounded corners, minimum width=3cm, minimum      height=1cm,text centered, draw=black, fill=black!5]
        % Specification of nodes (position, etc.)
        ...
        \draw [->] (evalBlock) -- ++(0,-1.25) node[xshift=-3.0cm, yshift=0.3cm] {$i=i+1$} coordinate(almostleft) -| (updateBlock.south);
